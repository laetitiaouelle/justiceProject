...
if(isset($_GET['niv']) && !empty($_GET['niv'])){
    if(isset($_GET['membre']) && !empty($_GET['membre'])){
        ...
        $del=$bdd->prepare('UPDATE equipe SET statut= :statut WHERE idMembre = :idMembre and niveau=:niv');
        $del->execute([
        'statut'=>1,
        ...
        if($del){
            //on selectionne dans la table equipe
            $a=$bdd->prepare('SELECT * FROM equipe WHERE niveau=:niveau and idMembre= :id');
            $a->execute([
                 'niveau'=>$_GET['niv'],
                     'id'=>$_GET['membre']
            ]);
