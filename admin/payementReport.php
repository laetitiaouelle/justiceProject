...
<?php require('partials/sidenav.php'); ?>
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <!-- DATA TABLE-->
                                <?php   
                                   $select = $bdd->prepare('SELECT * FROM detail_payement');
                                   $select->execute();
                                ...
                                <center><h3>Rapport Général</h3></center><br>
                                    <table class="table table-borderless table-data3">
                                        ...
                                        <?php foreach($datas as $data):
                                            ...
                                            $fmem->execute(['code'=>$data->codeDestinataire]);
                                            $rs=$fmem->fetch();
                                        ?>

                                            <tr>
                                                <td><?= $data->idPayement ?></td>
                                                <td><?= $data->date ?></td>
                                                <td><?= $data->montantPayement ?> Fcfa</td>
                                                <td class="process"><?= $rs['nom'].' '.$rs['prenom'] ?></td>
                                                <td><?= $data->typePayement ?></td>
                                            </tr>
                                        <?php endforeach ?>
                                        </tbody>
                                    </table>
    ...
    $upreport=$bdd->prepare("SELECT * FROM detail_payement WHERE date BETWEEN  :date1 AND :date2 ");
$upreport->execute([
    'date1'=>$date1,
    'date2'=>$date2
]);
                            ...
                            <div class="col-md-12">
                                ...
                                <div id="table" class="table-responsive m-b-40">
                                    <table  class="table table-borderless table-data3">
                                        <thead>
                                            <tr>
                                                Aucune activité n'a été trouvé pour ce jour &nbsp; &nbsp; <a href="payementReport.php" role="btn" class="btn btn-primary">Revenir...</a>
                                            </tr>
                                        </thead>
                               ...
                               <center> <h3>Rapport du <?= $_GET['date'] ?> </h3></center><br>
                                    <table   class="table table-borderless table-data3">
                                        ...
                                        <?php foreach($rest as $ret):
                                            $fmem=$bdd->prepare('SELECT * FROM membres WHERE code = :code');
                                            $fmem->execute(['code'=>$ret->codeDestinataire]);
                                            $rs=$fmem->fetch();
                                        ?>

                                            <tr>
                                                <td><?= $ret->idPayement ?></td>
                                                <td><?= $ret->date ?></td>
                                                <td><?= $ret->montantPayement ?></td>
                                                <td class="process"><?= $rs['nom'].' '.$rs['prenom'] ?></td>
                                                <td><?= $ret->typePayement ?>  Fcfa</td>
                                            </tr>
                                        <?php endforeach ?>
                                        </tbody>
                                    </table>
